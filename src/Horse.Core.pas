...
type
  ...
  IHorseDependencyResolver = interface
    ['{6EBE4EE4-B1F3-46B9-A512-E577C38B0B2E}']
...
{$IFEND}
    ...
    class property DependencyResolver: IHorseDependencyResolver read GetDependencyResolver write SetDependencyResolver;
...
class procedure THorseCore.SetDependencyResolver(const AValue: IHorseDependencyResolver);
begin
  FDependencyResolver := AValue;
end;

class function THorseCore.Get(const APath: string; AControllerClass: TClass; const AMethodName: string): THorseCore;
begin
  {$IFNDEF FPC}
  Result := THorseCore.Get(APath,
    ...
    var
      ...
      LMethod: TRttiMethod;
    begin
      if FDependencyResolver = nil then
        raise Exception.Create('IoC DependencyResolver is not registered.');

      LController := FDependencyResolver.GetService(AControllerClass);
      try
        LContext := TRttiContext.Create;
        LType := LContext.GetType(AControllerClass);
        if LType = nil then
          raise Exception.CreateFmt('Type not found for class %s', [AControllerClass.ClassName]);

        LMethod := LType.GetMethod(AMethodName);
        if LMethod = nil then
          raise Exception.CreateFmt('Method %s not found in Controller %s', [AMethodName, AControllerClass.ClassName]);

        LMethod.Invoke(LController, [Req, Res]);
      finally
        FDependencyResolver.Release(LController);
      end;
    end);
  {$ELSE}
